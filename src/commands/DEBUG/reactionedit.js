const MessageModel = require('../utils/message');
...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(argString.split(" ").length !== 1) return;
        // Check if the message exists.
        const { channel, author } = message;
        try {
            let fetchedMessage = channel.messages.cache.get(argString) || await channel.messages.fetch(argString);
            if(!fetchedMessage) 
                channel.send("Message not found.");
            else {
                // Check if the message exists in the DB.
                let msgModel = await MessageModel.findOne({ messageId: argString });
                if(msgModel) {
                    ...
                    try {
                        ...
                        if(choice.content === "add") {
                            ...
                            let collectorResult = await handleCollector(fetchedMessage, author, channel, msgModel, argString);
